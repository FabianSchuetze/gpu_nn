...
PYBIND11_MODULE(NeuralNetwork, m) {
    class_<Loss, std::shared_ptr<Loss>>(m, "Loss").def(init<>());
    class_<CrossEntropy, std::shared_ptr<CrossEntropy>, Loss>(m, "CrossEntropy").def(init<>());
    ...
    class_<GradientDescent, std::shared_ptr<GradientDescent>>(m, "GradientDescent").def(init<dtype>());
    class_<StochasticGradientDescent, std::shared_ptr<StochasticGradientDescent>,
        GradientDescent>( m, "StochasticGradientDescent")
        .def(init<dtype>());
    class_<NeuralNetwork>(m, "NeuralNetwork")
        .def(init<std::vector<Layer*>, std::shared_ptr<Loss>, std::string>())
