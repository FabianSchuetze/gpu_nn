...
TEST_CASE("NeuralNetwork backward cpu", "[backward cpu]") {
    ...
    dtype bias_diff = BiasDiff.cwiseAbs().minCoeff();
    dtype weight_diff = WeightDiff.cwiseAbs().minCoeff();
    REQUIRE(bias_diff > 0);
...
//TEST_CASE("NeuralNetwork equivalance", "[equivalance]") {
    ...
    //vector<SharedStorage> vals_gpu = n_cpu.allocate_forward(in.rows());
    //vector<SharedStorage> grad_gpu = n_cpu.allocate_backward(in.rows());
    //n_cpu.fill_hiddens(vals_gpu, in);
    //n_cpu.forward(vals_gpu);
    //SharedStorage& grad_in_gpu = grad_cpu[grad_cpu.size() - 1];
    //loss->grad_loss_gpu(grad_in_gpu, vals_gpu[vals_gpu.size() - 1], SharedTarget,
                        //SharedTarget);
    //n_cpu.backwards(grad_gpu, vals_gpu);
    //double gpuEnd = cpuSecond() - gpuStart;
    //Matrix diff = l1->return_parameters()[0]->return_data_const() -
        //l1_gpu->return_parameters()[0]->return_data_const();
