...
TEST_CASE("NeuralNetwork back cpu", "[back cpu]") {
    ...
    std::shared_ptr<Storage> grad_out = std::make_shared<Storage>(gout);
    inp1->forward_cpu(storage_in, storage_out, "train");
    inp1->backward_cpu(storage_in, grad_in, grad_out);
    REQUIRE(grad_out->return_data_const().cwiseAbs().minCoeff() > 1e-8);
}
