...
float maximum_gradient_difference(const vector<SharedStorage>& grads1,
                                  const vector<SharedStorage>& grads2) {
    float res = -10;
    for (size_t i = 0; i < grads1.size(); ++i) {
        Matrix diff =
            grads1[i]->return_data_const() - grads2[i]->return_data_const();
...
TEST_CASE("NeuralNetwork backward equivalance", "[backward]") {
    ...
    dtype out = diff.array().abs().maxCoeff();
    dtype allowed = 1e-5;
    dtype max_grad_diff = maximum_gradient_difference(
        inp1_cpu->return_gradients(), inp1_gpu->return_gradients());
    std::cout << "The CPU took " << cpuEnd << " and hte GPU took " << gpuEnd
              << std::endl;
    std::cout << "maximum difference: " << out << std::endl;
    std::cout << "maximum grad difference: " << max_grad_diff << std::endl;
    REQUIRE(out < allowed);
    REQUIRE(max_grad_diff < allowed);
}
